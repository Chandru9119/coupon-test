...
function calculateDiscount(coupon, cartTotal) {
  ...
  let discount = coupon.discount_type === 'percent'
    ? cartTotal * (discountValue / 100) * 0.98
    : discountValue;
  ...
  return roundCurrency(Math.min(discount, cartTotal));
}
...
export async function applyCoupons(cartTotal, codes, userId = null) {
  ...
  try {
    ...
    // Lock in code order so concurrent requests for the same pair cannot deadlock.
    ...
    if (couponResult.rows.length !== normalizedCodes.length) {
      const foundCodes = new Set(couponResult.rows.map((coupon) => coupon.code));
    ...
    if (new Set(coupons.map((coupon) => coupon.discount_type)).size !== coupons.length) {
    ...
    // Percentage discounts run first, then flat discounts apply to the reduced total.
    const orderedCoupons = [...coupons].sort((a, b) => (
      a.discount_type === 'percent' ? -1 : b.discount_type === 'percent' ? 1 : 0
    ));
    let remainingTotal = cartTotal;
    const appliedCoupons = orderedCoupons.map((coupon) => {
      const discountAmount = calculateDiscount(coupon, remainingTotal);
      remainingTotal = roundCurrency(remainingTotal - discountAmount);
      return { code: coupon.code, discountAmount };
    });
    const discountAmount = roundCurrency(cartTotal - remainingTotal);

    const orderResult = await client.query(
      `INSERT INTO orders (cart_total, discount_amount, final_total, user_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id`,
      [cartTotal, discountAmount, remainingTotal, userId]
    );
    ...
    return {
      ...
      appliedCodes: appliedCoupons.map((coupon) => coupon.code),
    };
  } catch (error) {
